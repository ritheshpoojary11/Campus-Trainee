...
<body>
  ...
  echo'
  <div class="content1">
<center> <h3 id="headi">ADD STAFF DETAILS</h3></center>
</div>
...
<table style="  border-spacing: 50px;">
  <tr>
    ...
    <td><p>Enter Username</p></td><td><input type=text placeholder="username" name="fname" value="';if(isset($_POST['fname'])){echo $_POST['fname'];}
    echo '" class=text required></td></tr>
    <tr><td><p>Enter Email</p></td><td><input type=email placeholder=email value="';if(isset($_POST['email'])){echo $_POST['email'];}
    echo '" name=email class=text required></td>
     <td><p>Enter Staff id</p></td><td><input type="text" value="';if(isset($_POST['staffid'])){echo $_POST['staffid'];}
    echo '" placeholder="staffid" name="staffid" class=text required></td></tr>
    <tr><td><p>Load Image</p></td><td><input type=file name=image class=file required></td>
    <td><p>Designation</p></td><td><input type=text value="';if(isset($_POST['desig'])){echo $_POST['desig'];}
    echo '" placeholder=designation name=desig class=text required></td></tr>
    <tr><td><p>Enter Department</p></td><td><input type="text" value="';if(isset($_POST['dept'])){echo $_POST['dept'];}
    echo '" placeholder="department" name="dept" class=text required></td>
     <td><p>Enter Qualification</p></td><td><input type="text" value="';if(isset($_POST['qual'])){echo $_POST['qual'];}
    echo '" placeholder="qualification" name="qual" class="text"></td></tr>
    <tr>  
     </div></td></tr>
     <tr><td colspan="2"></td>
  ...
  if (isset($_POST['submit'])) 
  {
    ...
    $qry="select * from staff where username='$username' || staff_id='$staffid'";
    $rslt=$db->query($qry);
    if($rslt->num_rows==0)
    {
        $qry="insert into staff(staff_id,name,designation,department,qual,username,password,email,image) values('$staffid','$name','$desig','$dept','$quali','$username','$password','$email','$image')";
        $rslt=$db->query($qry);
        ...
        if($rslt){
          echo"<script>Swal.fire({ 
                      ...
                      timer: 2000
                    }).then((result) => { window.location.href='add_staff.php' })</script>";
        }
        $to_email = $email;
    ...
    else
    {
      echo"<script>Swal.fire({
                              ...
                              icon: 'warning',
                              title: 'username or staffid already exist',
                              showConfirmButton: false,
                              timer: 2000
                            })</script>";
    }
